...
class LifecycleProjectionEngine:
    ...
    baseline_annual_revenue : float
        Current as-is annual rental income (from baseline rent roll).
    baseline_annual_expenses : float
        Current as-is annual operating expenses.
    stabilized_annual_revenue : float
        Projected gross annual revenue at full stabilization (from plan rent roll).
    annual_expense_ratio : float
        Operating expense ratio applied to EGI (e.g. 0.35 = 35%).
    annual_debt_service : float
        Total annual debt service across all facilities.
    vacancy_rate : float
        Assumed stabilized vacancy rate (e.g. 0.05 = 5%).
    annual_rent_increase : float
        Annual rent escalation rate (e.g. 0.03 = 3%).
    expense_growth_rate : float
        Annual expense escalation rate (e.g. 0.02 = 2%).
    construction_start_year : int or None
        Relative projection year when construction begins (1-based).
    construction_duration_years : int
        How many years construction takes.
    lease_up_months : int
        Months to ramp from 0% to stabilized occupancy after construction.
    carrying_cost_annual : float
        Annual carrying cost during construction (loan interest, insurance, etc.).
    exit_cap_rate : float
        Capitalization rate for terminal value calculation (e.g. 0.055 = 5.5%).
    disposition_cost_pct : float
        Selling/disposition costs as % of terminal value (default 0.02 = 2%).
    total_equity_invested : float
        Total equity basis for return calculations.
    debt_balance_at_exit : float
        Outstanding debt to be paid off at exit.
    projection_years : int
        Total number of years to project (default 10).
    """
    ...
    def __init__(
        ...
        # Legacy compatibility parameters (mapped internally)
    ...
    def project(self) -> List[YearProjection]:
        ...
        for year in range(1, self.projection_years + 1):
            ...
            if phase == "construction":
                ...
                # Use baseline expenses if available, otherwise expense ratio on baseline revenue
            ...
            # Debt service: $0 during construction if no baseline debt, full otherwise
            ds = round(self.annual_debt_service, 2)

            cash_flow = round(noi - ds, 2)
    ...
    def _calc_year(self, year: int, construction_end, lease_up_start, stabilization_year):
        ...
        if self.construction_start_year is None:
            # No development — property operates at baseline with rent escalation
            years_from_start = year - 1
            if self.baseline_revenue > 0:
                gpr = round(self.baseline_revenue * (1 + self.rent_escalation) ** years_from_start, 2)
                occupancy = 1.0 - self.vacancy_rate
            else:
                # Fall back to stabilized revenue if no baseline
